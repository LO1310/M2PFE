...
void displayNotificationsPage() {
    // Créer la page des notifications
    ...
    lv_obj_set_style_local_text_color(titleLabel, LV_LABEL_PART_MAIN, LV_STATE_DEFAULT, lv_color_hex(0xFFFFFF)); // Texte bleu
    ...
    lv_obj_set_event_cb(backBtn, [](lv_obj_t *obj, lv_event_t event) {
        if (event == LV_EVENT_CLICKED) {
            displayAppIconsPage(); // Retour à la grille des icônes
        }
    });

    // Ajouter manuellement des notifications
    int start_y = 60; // Position Y de départ
    for (int i = 0; i < 5; i++) {
        // Conteneur pour chaque notification
        lv_obj_t *item = lv_cont_create(notifPage, NULL);
        lv_obj_set_size(item, LV_HOR_RES - 40, 60);
        lv_obj_set_style_local_bg_color(item, LV_OBJ_PART_MAIN, LV_STATE_DEFAULT, lv_color_hex(0x333333)); // Fond gris
        lv_obj_set_style_local_radius(item, LV_OBJ_PART_MAIN, LV_STATE_DEFAULT, 10);
        lv_obj_align(item, NULL, LV_ALIGN_IN_TOP_MID, 0, start_y + (i * 70)); // Espacement vertical

        // Date
        lv_obj_t *dateLabel = lv_label_create(item, NULL);
        lv_label_set_text(dateLabel, "22 oct 2026");
        lv_obj_align(dateLabel, item, LV_ALIGN_IN_LEFT_MID, 10, -10);

        // Titre de la notification
        lv_obj_t *titleLabel = lv_label_create(item, NULL);
        lv_label_set_text(titleLabel, "Notifications ateliers");
        lv_obj_align(titleLabel, item, LV_ALIGN_CENTER, -40, -10);

        // Texte d'inscription
        lv_obj_t *subLabel = lv_label_create(item, NULL);
        lv_label_set_text(subLabel, "Inscrit : 58");
        lv_obj_align(subLabel, item, LV_ALIGN_IN_LEFT_MID, 10, 10);
        ...
        lv_obj_set_style_local_bg_color(actionBtn, LV_OBJ_PART_MAIN, LV_STATE_DEFAULT, lv_color_hex(0x00FF00)); // Vert

        lv_obj_t *actionLabel = lv_label_create(actionBtn, NULL);
        lv_label_set_text(actionLabel, LV_SYMBOL_PLUS);
        ...
        lv_obj_set_event_cb(actionBtn, [](lv_obj_t *obj, lv_event_t event) {
            if (event == LV_EVENT_CLICKED) {
                Serial.println("Action effectuée sur une notification !");
            }
        });
    }
...
void displayWelcomePage() {
    ...
    lv_obj_align(icon_logo, NULL, LV_ALIGN_IN_TOP_MID, 0, -10);
